...
def build_review_prompt(
    ...
    parts.extend([
        ...
        "## Response Format",
        "",
        "- GET API responses use TOON format (not JSON). Read them as structured plain text.",
        "- POST request/response bodies remain JSON.",
        "",
        "## Instructions",
        "",
...
def build_false_positive_review_prompt(
    ...
    parts = [
        ...
        "## Instructions",
        "",
        f"1. Retrieve the issue thread to see all opinions including the false_positive judgment:",
        f'   curl -H "X-Agent-Key: {agent_key}" {base}/api/sessions/{session_id}/issues/{issue_id}/thread',
        f"2. If needed, re-examine the code:",
        f'   curl -H "X-Agent-Key: {agent_key}" "{base}/api/sessions/{session_id}/files/{{path}}?start={{n}}&end={{n}}"',
        f"3. Submit your decision:",
        f"   curl -X POST {base}/api/sessions/{session_id}/issues/{issue_id}/opinions \\",
        f'     -H "Content-Type: application/json" \\',
        f'     -H "X-Agent-Key: {agent_key}" \\',
        f'     -d \'{{"model_id": "{model_id}", "action": "...", "reasoning": "...", "confidence": 0.8}}\'',
        "",
        "## Decision Options",
        "",

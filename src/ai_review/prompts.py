...
def build_review_prompt(
    ...
    parts.extend([
        ...
        "7. Submit each issue individually. Use heredoc with -b - to preserve backticks:",
        "   arv report -n \"이슈 제목\" -s severity --file path --lines start:end -b - <<'BODY'",
        "   description here (Markdown)",
        "   BODY",
        "   For suggestions, add before -b -: --sb \"제안\" or --sf /tmp/sugg.md",
...
def build_deliberation_prompt(
    ...
    parts.extend([
        ...
        "   arv opinion {issue_id} -a {action} -s {severity} -c {confidence} -b - <<'BODY'",
        "   reasoning here",
        "   BODY",
...
def build_agent_response_prompt(
    ...
    parts = [
        ...
        "   arv respond {issue_id} -a accept -b - <<'BODY'",
        "   reasoning here",
        "   BODY",
        "",
        "## Response Guidelines",
...
def build_verification_prompt(
    ...
    parts.extend([
        ...
        "   arv report -n \"새 이슈 제목\" -s severity --file path --lines start:end -b - <<'BODY'",
        "   설명",
        "   BODY",
...
def build_false_positive_review_prompt(
    ...
    parts = [
        ...
        f"   arv opinion {issue_id} -a {{action}} -c 0.8 -b - <<'BODY'",
        "   reasoning here",
        "   BODY",
        "",
        "## Decision Options",
